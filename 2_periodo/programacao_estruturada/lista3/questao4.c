...
int main(void){
    printf("Contador de Bilhetes Clonados\n");
    while (1){
        int N, M;
        scanf("%d %d", &N, &M);
        if((N == M) && (N == 0)) break;
        else if ((N < 1) || (N > 10000) || (M < 1) || (M > 20000)) {
            printf("Valores inválidos. Tente novamente.\n");
            continue;
        }

        int freq_bilhetes[10001] = {0}, bilhetes_clonados = 0;

        for (int i = 0; i < M; i++) {
            int bilhete;
            scanf("%d", &bilhete);

            if(freq_bilhetes[bilhete] == 1) {
                bilhetes_clonados++;
            }

            freq_bilhetes[bilhete]++;
        }
