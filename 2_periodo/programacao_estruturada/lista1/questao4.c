...
int main() {
    ...
    scanf("%d", &n);
    for (int i = 1; i <= n; i++) {
        printf("Insira o número de figurinhas de Ricardo e Vicente (Formato: F1 F2):\n");
        int f1, f2;
        while (1) {
            scanf("%d %d", &f1, &f2);
            if ((1 <= f1 && f1 <= 1000)&&(1 <= f2 && f2 <= 1000)){
                break;
            }
        }
